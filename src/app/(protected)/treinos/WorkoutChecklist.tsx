...
export default function WorkoutChecklist({ initialLogs }: WorkoutChecklistProps) {
  ...
  return (
    <div className="w-full">
      <div className="w-full grid grid-cols-7 mb-6 bg-card/50 rounded-lg p-1">
        {[0, 1, 2, 3, 4, 5, 6].map(dayIdx => {
          ...
          return (
            <button
              key={dayIdx} 
              onClick={() => setActiveTab(dayIdx.toString())}
              className={`flex flex-col items-center justify-center h-auto py-2 rounded-md transition-all ${isActive ? 'bg-background shadow-sm' : 'hover:bg-background/50'} ${isToday && !isActive ? 'border-b-2 border-fire-500 rounded-none' : ''}`}
            >
      ...
      {WORKOUT_ROUTINE.filter(d => d.dayOfWeek.toString() === activeTab).map(day => (
        <div key={day.dayOfWeek} className="animate-fade-in">
          <div className="mb-6">
